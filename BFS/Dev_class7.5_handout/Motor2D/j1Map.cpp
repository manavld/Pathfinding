...
void j1Map::PropagateBFS()
{
	...
	iPoint neighbor1, neighbor2, neighbor3, neighbor4 = frontier.start->data;
	if (frontier.Count() != 0) {
		frontier.Pop(visited.start->data);
		neighbor1.x += 1;
		neighbor2.x -= 1;
		neighbor3.y += 1;
		neighbor4.y -= 1;
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	//return 0 <= x <= 25 && 0 <= y <= 25 && map_file.child("tileset").child("tile").attribute("id").value=="0";
	return 0 <= x <= 25 && 0 <= y <= 25;
}
